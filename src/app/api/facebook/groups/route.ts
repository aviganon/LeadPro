...
export async function GET(req: NextRequest) {
  ...
  const own = requireMatchingUser(auth.uid, userId)
  ...
  try {
    ...
    if (forceRefresh) {
      const tokenData = await getFacebookAccessTokenForUser(resolvedUserId)
      ...
      const existingSnap = await db.collection('fb_groups').doc(resolvedUserId).get()
      ...
      await db.collection('fb_groups').doc(resolvedUserId).set({
    ...
    const snap = await db.collection('fb_groups').doc(resolvedUserId).get()
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const resolvedUserId = userId as string
    const db = getAdminFirestore()
    const ref = db.collection('fb_groups').doc(resolvedUserId)
