...
// מחזיר { questions: { prompt, options, answer, explanation }[] }
export async function POST(req: NextRequest) {
  ...
  let body: { subject?: string; grade?: number; topic?: string; count?: number; lang?: string }
  try {
    body = await req.json()
  } catch {
  ...
  const subject = (body.subject ?? '').toString().slice(0, 60) || 'general'
  const grade = Number.isFinite(body.grade) ? body.grade : 1
  ...
  try {
    ...
    const message = await anthropic.messages.create({
      ...
      max_tokens: 1500,
      system:
        `You generate age-appropriate multiple-choice practice questions for school students. ` +
      ...
      messages: [{
        role: 'user',
        content: `Subject: ${subject}. Grade: ${grade}.${topic ? ` Topic: ${topic}.` : ''} Generate ${count} questions.`,
      }],
    })
